...
if(class_exists('CF7AU_AURASTRIDE_API')){
    ...
    $arrPrams = array("form_code"=>$formVal);
    ...
    if( !empty( $af_form_fields_arr ) && $af_form_fields_arr['status'] == "success"  ){
        ...
        if( !empty( $form_fields_array ) ){
            ...
            foreach ($form_fields_array as $d_key => $form_field) {
                ...
                $field_api_id = 'cf7aura_form_af_api_key_'.$formId.'_'.$i;
                $field_cf7_id = 'cf7aura_form_cf7_key_'.$formId.'_'.$i;
                $selected = '';
                ...
                ?><tr data-id="<?php print esc_attr($i);?>">
                    <td class="col_gen_field">
                        ...
                        ?>
                                ...
                                <input type="hidden" name="cf7aura_form_fields[<?php echo esc_attr($formId); ?>][api_form_key][<?php echo esc_attr($d_key); ?>]['from']" value="<?php echo esc_attr($d_key); ?>" />
                                <input type="hidden" name="cf7aura_form_fields[<?php echo esc_attr($formId); ?>][api_form_key][<?php echo esc_attr($d_key); ?>]['to']" value="<?php echo esc_attr($d_key); ?>" />
                        <?php		
                            }
                            else{
                        ?>
                                <input type="hidden" name="cf7aura_form_fields[<?php echo esc_attr($formId); ?>][api_form_key][]" value="<?php echo esc_attr($d_key); ?>" />
                        <?php
                            }
                        ?>
                        ...
                        if( $form_field['type'] == "DR" || $form_field['type'] == "TR" ){
                            ...
                            <table>
                                <tr>
                                    ...
                                    <td style="width:50%;vertical-align:top;">
                                        <select data-id="<?php print esc_attr($i);?>" name="cf7aura_form_fields[<?php echo esc_attr($formId); ?>][cf7_form_key][<?php echo esc_attr($d_key); ?>][from]"  id="<?php print esc_attr($field_cf7_id);?>" data-fieldType="default" <?php if( $form_field['required'] == "Y" ) { echo 'data-required="true"'; echo " required"; }?> >
                    ...
                    <!-- Check for the field if FROM - TO type -->
                        ...
                        <select data-id="<?php print esc_attr($i);?>" name="cf7aura_form_fields[<?php echo esc_attr($formId); ?>][cf7_form_key][]"  id="<?php print esc_attr($field_cf7_id);?>" data-fieldType="default" <?php if( $form_field['required'] == "Y" ) { echo 'data-required="true"'; }?>>
                                <option value=""><?php esc_html_e( 'N/A', 'integrate-cf7-to-aurastride' );?></option><?php
                                foreach ($arr_form_tags as $Fname => $fVal){
                                    if( empty($Fname) ){
                                        continue;
                                    ...
                                    //Check if the Custom field type is not File then change the selection and do not allow file fields
                                    if( $form_field['type'] != "FL" && $fVal["field_type"] == "file" ){
                                        continue;
                                    }
